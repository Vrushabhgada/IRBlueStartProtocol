...
/// @endcond
                      ...
                      uint8_t max_skip, uint16_t noise_floor)
  {
         ...
         offset += 2)
    {
      ...
      if (decodeLG(results, offset, kLg32Bits, true))
        return true;
#endif
#if DECODE_GICABLE
...
#if DECODE_BLUESTAR
      DPRINTLN("Attempting BLUESTAR decode");
      if (decodeBluestar(results, offset))
        return true;
#endif
      ...
      if (decodeToshibaAC(results, offset, kToshibaACBitsLong))
        return true;
      DPRINTLN("Attempting Toshiba AC 56bit decode");
      if (decodeToshibaAC(results, offset, kToshibaACBitsShort))
        return true;
#endif
#if DECODE_MIDEA
      ...
      if (decodeSamsungAC(results, offset, kSamsungAcBits))
        return true;
#endif
      ...
      if (decodePanasonicAC(results, offset, kPanasonicAcShortBits))
        return true;
#endif
#if DECODE_LUTRON
